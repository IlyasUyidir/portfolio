...
class BudgetControllerSecurityTest {
    ...
    void setUp() throws ServletException, IOException {
        // Ensure the rate limit filter passes the request along the chain
        doAnswer(invocation -> {
        ...
        // For the valid token test, we must mock budgetService to return an empty list or some response 
        // to avoid null pointers or 404s depending on what the controller does, 
        // though just returning 200 is sufficient if the controller handles null gracefully.
        when(budgetService.getBudgetsByMonth(any(), any())).thenReturn(Collections.emptyList());
    }
