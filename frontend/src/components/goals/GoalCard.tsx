...
export const GoalCard: React.FC<GoalCardProps> = ({ goal, progress, onContribute, onDelete }) => {
  ...
  return (
    <>
      <div className="bg-bg-card p-6 rounded-xl border border-border-subtle flex flex-col justify-between h-full gap-8">
        <div className="flex flex-col gap-8 py-2">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-end text-sm">
              <span className="text-text-primary font-semibold text-base">{formatCurrency(goal.currentAmount * 100)}</span>
        ...
        {goal.status !== 'ATTEINT' && (
          <button
            ...
            className="w-full py-3 rounded-lg bg-primary text-bg-base font-bold hover:bg-primary-hover transition-colors shadow-sm"
          >
            Contribuer
          </button>
        )}
